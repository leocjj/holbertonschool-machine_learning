...
class NeuralNetwork:
    ...
    def forward_prop(self, X):
        ...
        """
        self.A1 = sigmoid(np.matmul(self.W1, X) + self.b1)
        return self.A1
    ...
    def gradient_descent(self, X, Y, A, alpha=0.05):
        ...
        m = Y.shape[1]
        dZ = A - Y
        self.W1 -= (alpha * ((1 / m) * np.matmul(X, dZ.T))).T
        self.b1 -= (alpha * ((1 / m) * np.sum(dZ)))
    ...
    def train(self, X, Y, iterations=5000, alpha=0.05, verbose=True, graph=True, step=100):
        ...
        costs = []
        # steps = [i for i in range(int((iterations + 1) / step))]
        steps = np.arange(0, int((iterations + 1) / step), step)
        for i in range(iterations + 1):
            self.forward_prop(X)
            if verbose and i % step == 0:
                cost = self.cost(Y, self.A1)
                print("Cost after {} iterations: {}".format(i, cost))
                costs.append(cost)
            self.gradient_descent(X, Y, self.A1, alpha)
